 ...
 * Improvements by Dmitry @bpystep Bagaev 2018
 *
...
(function(factory) {
    ...
    var Datetimepicker = function(element, options) {
        ...
        this.component = this.element.is('.date') ? (this.bootcssVer === 3 ?
            this.element.find('.input-group-addon .glyphicon-th, .input-group-addon .glyphicon-time, .input-group-addon .glyphicon-remove, .input-group-addon .glyphicon-calendar, .input-group-addon .fa-calendar, .input-group-addon .fa-clock-o').parent() :
        ...
        this.icons = {
            leftArrow : this.fontAwesome ? 'fa-arrow-left' : (this.bootcssVer === 3 ? 'fas fa-long-arrow-alt-left' : 'icon-arrow-left'),
            rightArrow: this.fontAwesome ? 'fa-arrow-right' : (this.bootcssVer === 3 ? 'fas fa-long-arrow-alt-right' : 'icon-arrow-right')
        };
    ...
    Datetimepicker.prototype = {
        ...
        keydown              : function(e) {
            ...
            switch (e.keyCode) {
                //escape
                case 27: this.hide(); e.preventDefault(); break;
                //left, right
                case 37:
                case 39: {
    ...
    var DPGlobal = {
        ...
        getDefaultFormat: function(type, field) {
            if (type === 'standard') {
                return field === 'input' ? 'yyyy-mm-dd hh:ii' : 'yyyy-mm-dd hh:ii:ss';
            } else if (type === 'php') {
                return field === 'input' ? 'Y-m-d H:i' : 'Y-m-d H:i:s';
            } else {
        ...
        parseDate       : function(date, format, language, type, timezone) {
            ...
            if (/^[-+]\d+[dmwy]([\s,]+[-+]\d+[dmwy])*$/.test(date)) {
                ...
                for (var i = 0; i < parts.length; i++) {
                    ...
                    dir = parseInt(part[1]);
                    switch (part[2]) {
                        case 'd': date.setUTCDate(date.getUTCDate() + dir); break;
                        case 'm': date = Datetimepicker.prototype.moveMonth.call(Datetimepicker.prototype, date, dir); break;
                        case 'w': date.setUTCDate(date.getUTCDate() + dir * 7); break;
                        case 'y': date = Datetimepicker.prototype.moveYear.call(Datetimepicker.prototype, date, dir); break;
                    }
                }
        ...
        formatDate      : function(date, format, language, type, timezone) {
            ...
            if (type === 'standard') {
                val = {
                    ...
                    yyyy: date.getUTCFullYear(), // month
                    m   : date.getUTCMonth() + 1,
                    M   : dates[language].monthsShort[date.getUTCMonth()],
                    MM  : dates[language].months[date.getUTCMonth()], // day
                    d   : date.getUTCDate(),
                    D   : dates[language].daysShort[date.getUTCDay()],
                    DD  : dates[language].days[date.getUTCDay()],
                    p   : (dates[language].meridiem.length === 2 ? dates[language].meridiem[date.getUTCHours() < 12 ? 0 : 1] : ''), // hour
                    h   : date.getUTCHours(), // minute
                    i   : date.getUTCMinutes(), // second
                    s   : date.getUTCSeconds(), // timezone
                    z   : timezone
                };
                ...
                val = {
                    ...
                    Y: date.getUTCFullYear(), // month
                    F: dates[language].months[date.getUTCMonth()],
                    M: dates[language].monthsShort[date.getUTCMonth()],
                    n: date.getUTCMonth() + 1,
                    t: DPGlobal.getDaysInMonth(date.getUTCFullYear(), date.getUTCMonth()), // day
                    j: date.getUTCDate(),
                    l: dates[language].days[date.getUTCDay()],
                    D: dates[language].daysShort[date.getUTCDay()],
                    w: date.getUTCDay(), // 0 -> 6
                    N: (date.getUTCDay() === 0 ? 7 : date.getUTCDay()),       // 1 -> 7
                    S: (date.getUTCDate() % 10 <= dates[language].suffix.length ? dates[language].suffix[date.getUTCDate() % 10 - 1] : ''), // hour
                    a: (dates[language].meridiem.length === 2 ? dates[language].meridiem[date.getUTCHours() < 12 ? 0 : 1] : ''),
                    g: (date.getUTCHours() % 12 === 0 ? 12 : date.getUTCHours() % 12),
                    G: date.getUTCHours(), // minute
                    i: date.getUTCMinutes(), // second
        ...
        convertViewMode : function(viewMode) {
            switch (viewMode) {
                case 4       :
                case 'decade': viewMode = 4; break;
                case 3       :
                case 'year'  : viewMode = 3; break;
                case 2       :
                case 'month' : viewMode = 2; break;
                case 1       :
                case 'day'   : viewMode = 1; break;
                case 0       :
                case 'hour'  : viewMode = 0; break;
            }
